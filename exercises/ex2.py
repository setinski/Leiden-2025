...
def in_lattice(B, v):
    ...
    :param B: A square (n x n) NumPy array representing a lattice basis.
        Note that the rows of B correspond to vectors of the basis. 
...
def simple_rounding(B, t):
    ...
    """
    # tprime = np.linalg.inv(B) @ t -> Slower than:
    
    # tprime = np.linalg.solve(B,t)
    # vprime = np.round(tprime)
    # return B @ vprime
    
    return np.round(np.linalg.solve(B.T,t)) @ B
...
def compare_norm_distrib(B, num_samples):
    ...
    for i in range(num_samples):
        ...
        data_SR[i] = np.linalg.norm(samples @ B)
