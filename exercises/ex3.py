...
def simple_enumeration(B, t, l):
	...
	n = B.shape[0]
	tp = np.linalg.solve(B.transpose(),t)
	for v in enumeration(tp,l):
		w = B @ v
		if np.linalg.norm(w-t) < np.linalg.norm(c-t):
			c = w
...
def fincke_pohst_1d_enumeration(b1, x, t, r):
    ...
    """
	S = []
	high = math.ceil(r/(b1 @ b1) - ((x-t) @ b1)/(b1 @ b1))
	low = math.floor(-r/(b1 @ b1) - ((x-t) @ b1)/(b1 @ b1))
...
def fincke_pohst_enumeration(B, t, r):
	...
	b1 = B[0]
	if n == 1:
		return fincke_pohst_1d_enumeration(b1,np.zeros_like(t),t,r)
	S = []
	Bp = np.zeros((n-1,n))
	t_proj = t- orth_proj(t,b1)
	for i in range(n-1):
		Bp[i] = B[i+1] - orth_proj(B[i+1],b1)
	Sp = fincke_pohst_enumeration(Bp,t_proj,r)
	for s in Sp:
		ro = math.sqrt(r*r - np.linalg.norm(s-t_proj)**2)
		y = np.linalg.lstsq(Bp.T, s, rcond=None)
		x = y @ B[1:]
	S.extend(fincke_pohst_1d_enumeration(b1,x,t,r))
	return S
